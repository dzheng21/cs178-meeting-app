<script>
	...
	function responsiveText(windowWidth) {
		if(windowWidth <= 800) {
			...
			for(let i = 0; i < times.length; i++) {
				times[i].innerText = shortTimeFormatter.format(timeArray[times[i].id.slice(1)])
			}
		} else {
			...
			for(let i = 0; i < times.length; i++) {
				times[i].innerText = dateFormatter.format(timeArray[times[i].id.slice(1)])
			}
		}
	...
	function registerStart(e, id) {
		...
		if (document.getElementById(id).style.background == virtualColor || document.getElementById(id).style.background == inPersonColor) {
			selecting = false;
		} else {
			selecting = true;
		}
				...
				for (let j = startTime; j <= currentTime; j++) {
					if (selecting) {
						...
						if(color == inPersonColor) {
							document.getElementById(i + "_" + j).style.boxShadow = "0 0 8px 2px #32d97759";
						} else {
							document.getElementById(i + "_" + j).style.boxShadow = "0 0 8px 2px #3796e659";
						}
					} else {
	...
	function registerMove(e, id) {
		if (dragging) {
			...
			if (getTimeCodeFromID(selectionArray[0]) === getTimeCodeFromID(selectionArray[1])) {
				deleteElement("endTime");
				showTime(selectionArray[1], "startTime");
			} else if (selectionArray[0] !== selectionArray[1]) { // If 
				if(parseInt(selectionArray[0].slice(2)) > parseInt(selectionArray[1].slice(2))) {
					showTime(selectionArray[1], "startTime");
			...
			if(innerWidth <= 640) {
				...
				selection.style.left = document.getElementById("sidebar").getBoundingClientRect().width + 2 + (document.getElementById("Sunday").getBoundingClientRect().width * startWeek) + "px";
			}
			if(innerWidth <= 640) {
				selection.style.top = startTime * document.getElementById(id).getBoundingClientRect().height + (numberOfGapsFromStart * hourMargin) + "px";
			} else {
				selection.style.top = startTime * document.getElementById(id).getBoundingClientRect().height + 2 + (numberOfGapsFromStart * hourMargin) + "px";
			}
			if(innerWidth <= 640) {
				selection.style.width = document.getElementById("Sunday").getBoundingClientRect().width * (currentWeek - startWeek + 1) - 4 + "px";
			} else {
				selection.style.width = document.getElementById("Sunday").getBoundingClientRect().width * (currentWeek - startWeek + 1) - 8 + "px";
			}
			
			selection.style.height = document.getElementById(id).getBoundingClientRect().height * (currentTime - startTime + 1) + (numberOfGaps * hourMargin) + 2 + "px";
			document.getElementById("selection").style.display = "block";
			if (selecting) {
				document.getElementById("selection").style.background = selectColor;
				document.getElementById("selection").style.boxShadow = "0 0 10px 3px #32d97733";
				document.getElementById("selection").style.border = "2px solid " + selectBorder;
			} else {
				document.getElementById("selection").style.background = deleteColor;
				document.getElementById("selection").style.boxShadow = "0 0 10px 3px #f2513f40";
				document.getElementById("selection").style.border = "2px solid " + deleteBorder;
			}
			...
			if(startWeek == currentWeek) {
				document.getElementById("tipText").innerHTML = daysOfTheWeek[startWeek] + " " + dateFormatter.format(timeArray[startTime]) + " &#8212; " + dateFormatter.format(newEndTime.setMinutes(timeArray[currentTime].getMinutes() + 15)) + " Selected";
			} else {
	...
	function registerMouseOver(e, id) {
		if(!dragging) {
			...
			if(document.getElementById(id).style.background == inPersonColor || document.getElementById(id).style.background == virtualColor) {
		...
		if(hour != "h0") {
			...
			document.getElementById(hour).style.boxShadow = "0 10px 10px -8px #1ed9b359";
	...
	function registerEnd(e) {
		if (selecting && dragging) {
			...
			let selectionRect = document.getElementById("selection").getBoundingClientRect();
			...
			if (currentWeek - startWeek > 2) {
				...
				tooltip.style.left = selectionRect.left + (selectionWidth / 2) - (tooltip.offsetWidth / 2) + "px";
				tooltip.style.top =
					selectionRect.top +
					selectionHeight / 2 -
...
<main on:mouseup={(event) => registerEnd(event)}>
	<div id="tooltip">
		<div class="tooltip-header">
			...
			<a class="close" on:click={disableSelection}></a>
		</div>
		<div class="tooltip-body">
			<ul>
				<li>
					<button class="inPerson" on:click={(event) => confirmSelection(event, inPersonColor)}>Available</button>
				</li>
				<li>
					<button class="virtual" on:click={(event) => confirmSelection(event, virtualColor)}>Only Available Virtually</button>
				</li>
			</ul>
		</div>
	</div>
	...
	<div class="col">
		...
		<div class="row">
					...
					<div class="cal-body">
						<div class="cal-content">
									...
									<div class="hour">
										{#each hourArray as time, j}
											...
											{:else if time.getMinutes() == 0}
												<div class="cal-time visible" id={"h" + (j + (k * 4))}>{dateFormatter.format(time)}</div>
											{:else}
												<div class="cal-time"></div>
											{/if}
										{/each}
							...
							<div class="cal-days" id="weekWrapper">
								<div id="selection" />
									{#each daysOfTheWeek as day, i}
										<div class="cal-day" id={day} on:mouseover={(event) => registerMouseOverDay(event, i)} on:mouseout={(event) => registerMouseOutOfDay(event, i)}>
											{#each timeArrayHours as hourArray, k}
												<div class="hour">
													{#each hourArray as time, j}
														{@const id = i + "_" + (j + (k * 4))}
			...
			<div class="legend">
				<span><div class="inPersonColor"></div> Available</span>
				<span><div class="virtualColor"></div> Available Only Virtually</span>
			</div>
	</div>	
</main>
...
<style>
	...
	#tooltip {
		...
		opacity: 0;
		transform: scale(0);
		transition: opacity 0.08s ease-out, transform 0.15s cubic-bezier(.17,.67,.15,1.07);
		text-align: center;
	}
	...
	.cal-day.textHeading {
		...
		margin-top: 0;
		margin-bottom: 10px;
		transition: transform 0.15s cubic-bezier(.17,.67,.19,1.9), box-shadow 0.15s ease-out, background 0.15s ease-out;
		font-size: 18px;
	}
	...
	@media (max-width: 640px) {
		...
		.cal-sidebar .hour .cal-time.visible, .cal-hidden-times .cal-time {
			font-size: 12px;
		}
